...
class SubAgentManager {
  ...
  addSubAgent(agent) {
    ...
    registry_1.AgentRegistry.getInstance().registerSubAgent(this.agentName, agent.id);
  }
  /**
   * Remove a sub-agent
   */
  removeSubAgent(agentId) {
    ...
    registry_1.AgentRegistry.getInstance().unregisterSubAgent(this.agentName, agentId);
  ...
  unregisterAllSubAgents() {
    ...
    for (const agent of this.subAgents) {
      registry_1.AgentRegistry.getInstance().unregisterSubAgent(this.agentName, agent.id);
    }
  }
  /**
   * Get all sub-agents
  ...
  createDelegateTool(options = {}) {
    return (0, tool_1.createTool)({
      ...
      parameters: zod_1.z.object({
        ...
        targetAgents: zod_1.z
          .array(zod_1.z.string())
          .describe("List of agent names to delegate the task to"),
        context: zod_1.z
  ...
  getSubAgentDetails() {
    return this.subAgents.map((subAgent) => {
      ...
      if (fullState.subAgents && fullState.subAgents.length > 0) {
        fullState.subAgents = fullState.subAgents.map((nestedAgent) => {
